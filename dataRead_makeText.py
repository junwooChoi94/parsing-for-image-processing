...
cars = ["car", "bus", "truck", "excavator", "forklift", "ladder truck", "unknown car", "bicycle", "motorbike"]

# root_path = "D:/backup/DataSet"
target_path = r"D:\backup\DataSet\UniTrain19_Asan"  #########경로변환
...
for x in os.listdir(target_path):
    if x.endswith('xml'):
        empty = os.path.join(target_path, x)
        path_list.append(empty)

        total_xml += 1
        tree = ET.parse(os.path.join(target_path, x))  # x에 대한정보를 가져온다
...
print("unspecified object:" + str(other))

####################

print("person width  " + " average:" + str(a) + "  standard deviation:" + str(d) + "  min:" + str(
    np.min(x)) + "  max:" + str(np.max(x)))
print("person height  " + " average:" + str(b) + "  standard deviation:" + str(e) + "  min:" + str(
    np.min(y)) + "  max:" + str(np.max(y)))
print("person width*height  " + " average:" + str(c) + "  standard deviation:" + str(ff) + "  min:" + str(
    np.min(z)) + "  max:" + str(np.max(z)))

print("car width  " + " average:" + str(a1) + "  standard deviation:" + str(d1) + "  min:" + str(
    np.min(x1)) + "  max:" + str(np.max(x1)))
print("car height  " + " average:" + str(b1) + "  standard deviation:" + str(e1) + "  min:" + str(
    np.min(y1)) + "  max:" + str(np.max(y1)))
print("car width*height  " + " average:" + str(c1) + "  standard deviation:" + str(f1) + "  min:" + str(
    np.min(z1)) + "  max:" + str(np.max(z1)))
